 ...
 */
class MaybeTask extends DefaultTask {
    @InputDirectory
    File inputDir
    ...
    def execute(IncrementalTaskInputs inputs) {
        MaybeExtension retrolambda = project.retrolambda
        ...
        if (!inputs.incremental || !changes.isEmpty()) {
            project.javaexec {
                ...
                if (!project.retrolambda.onJava8) {
                    ...
                    if (!me.tatarka.RetrolambdaPlugin.checkIfExecutableExists(java)) {
                        throw new ProjectConfigurationException("Cannot find executable: $java", null)
                    }
                    executable java
                }

                def bytecodeVersion = me.tatarka.RetrolambdaPlugin.javaVersionToBytecode(javaVersion)
